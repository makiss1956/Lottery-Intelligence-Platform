...
from src.importers.eurojackpot_importer import (
    EurojackpotImporter,
)
...
def run_pipeline() -> Dict[str, Any]:
    ...
    logger.info(
        "STEP 2 - Synchronize historical CSV"
    )
    ...
    logger.info(
        "CSV synchronization inserted %d draws",
        inserted,
    )
    ...
    logger.info(
        "STEP 3 - Retrieve latest live draw"
    )
    ...
    if latest_draw is None:
        ...
        raise RuntimeError(
            "Could not retrieve latest Eurojackpot draw."
        )

    latest_date = latest_draw[
        "draw_date"
    ]
    ...
    for prediction in predictions:
        ...
        evaluated_prediction = (
            Backtester.evaluate_prediction(
                predicted_mains=prediction.get(
                    "predicted_primary",
                    [],
                ),
                predicted_euros=prediction.get(
                    "predicted_euro",
                    [],
                ),
    ...
    logger.info(
        "STEP 7 - Generate 3 + 1 prediction"
    )
    ...
    primary_candidates = sorted(
        prediction["primary_candidates"]
    )

    joker_candidates = sorted(
        prediction["euro_candidates"]
    )
    ...
    # If a prediction already exists for this target,
    ...
    if db.prediction_exists(
        ...
        existing = next(
            (
                item
                for item in db.get_predictions(
                    limit=100
                )
                if item.get(
                    "for_draw_date"
    ...
    logger.info(
        "Prediction: Main=%s | Joker=%s",
        primary_candidates,
        joker_candidates[0],
    )

    logger.info(
        "Previous prediction evaluated: %s",
        evaluated_prediction is not None,
    )
