...
class ProbabilityPredictor:
    ...
    def predict_candidate_set(
        ...
        if primary_count != 3:
            raise ValueError("This model requires exactly 3 main numbers.")

        if euro_count != 1:
            raise ValueError("This model requires exactly 1 Joker/Euro number.")
        ...
        primary_candidates = [
            number for number, _ in sorted_primary[:primary_count]
        ]
        ...
        euro_candidates = [
            number for number, _ in sorted_euro[:euro_count]
        ]
