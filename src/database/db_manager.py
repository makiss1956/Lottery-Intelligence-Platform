...
class DBManager:
    ...
    def insert_prediction(self, prediction: Dict[str, Any]) -> bool:
        primary = sorted(set(prediction["predicted_primary"]))
        euro = sorted(set(prediction["predicted_euro"]))
        if len(primary) != 3:
            raise ValueError(
        ...
        with self._get_connection() as connection:
            cursor = connection.execute(
                ...
                (prediction_date, for_draw_date, model_name,
                ...
                (
                    ...
                    prediction.get("model_name", "composite_frequency_delay"),
                    json.dumps(primary),
                    json.dumps(euro),
                ),
            )
    ...
    def save_prediction(
        ...
        prediction = {
            ...
            "model_name": metadata.get("method", "composite_frequency_delay"),
    ...
    def get_predictions(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        sql = """
            SELECT id, prediction_date, for_draw_date, model_name,
                   predicted_primary, predicted_euro
            FROM predictions
    ...
    def _row_to_draw(cls, row: sqlite3.Row) -> Dict[str, Any]:
        return {
            ...
            "primary_numbers": cls._decode_numbers(row["primary_numbers"]),
            "euro_numbers": cls._decode_numbers(row["euro_numbers"]),
        }

    @classmethod
    def _row_to_prediction(cls, row: sqlite3.Row) -> Dict[str, Any]:
        return {
            "id": row["id"],
            "prediction_date": row["prediction_date"],
            "for_draw_date": row["for_draw_date"],
            "model_name": row["model_name"],
            "predicted_primary": cls._decode_numbers(row["predicted_primary"]),
            "predicted_euro": cls._decode_numbers(row["predicted_euro"]),
        }
